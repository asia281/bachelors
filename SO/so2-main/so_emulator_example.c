...
static const uint16_t code_base[MEM_SIZE] = {
...
static const uint16_t code_inc2[MEM_SIZE] = {
...
static void single_core_simple_test(void) {
  dump_cpu_state(0, so_emul(code_base, data, 1, 0), data);
  dump_cpu_state(0, so_emul(code_base, data, 1, 0), data);
  dump_cpu_state(0, so_emul(code_base, data, 1, 0), data);
  dump_cpu_state(0, so_emul(code_base, data, 1, 0), data);
 // dump_cpu_state(0, so_emul(code_base, data, 5, 0), data);
...
int main(int argc, char *args[]) {
    multi_core_inc_test(strtoumax("10240", NULL, 10));
    return 0;
  if (argc == 1)
